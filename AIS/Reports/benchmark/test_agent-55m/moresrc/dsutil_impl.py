...
def v43_binary_search(arr: list, target) -> int:
    low, high = 0, len(arr) - 1
    while low < high:
        mid = (low + high) // 2
        if arr[mid] == target:
...
def v44_flatten(nested_list: list) -> list:
    result = []
    for item in nested_list:
        if isinstance(item, list) and item:
            result.extend(v44_flatten(item))
        else:
            result.append(item)
    return result


def v45_merge_dicts(d1: dict, d2: dict) -> dict:
    d1.update(d2)
    return d1


def v55_chunk_list(lst: list, size: int) -> list:
    if size < 1:
        raise ValueError("청크 크기는 1 이상이어야 합니다.")
